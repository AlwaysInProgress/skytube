...
class SkyTubeSim:
	...
	def __init__(self) -> None:
		...
		self.impactTime = 30
		self.maxTime = 30
		self.initialVelocity = -53; #terminal velocity in m/s
		...
		self.pressureDrag = 0 #N
		self.buoyancyForce = 0 #N
	...
	def step_human(self):
			...
			self.pressureDrag = 0.5*self.fluidDensity[i]*self.velocity[i-1]**2*self.humanFrontalArea*self.C_D #N upward
			buoyancyForce = self.fluidDensity[ i ]*self.humanVolume*self.gravity; #N upward
			dv = (self.pressureDrag + buoyancyForce - self.gravityForce)/self.humanMass*self.dt
	...
	def run(self):
		while (True):
			print("Current Height: ",self.height[self.currentIndex-1])
			if self.step_human():
				break
